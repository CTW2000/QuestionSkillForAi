...
- **12 条通用变换**（`T1`–`T12`）—— 把弱问法改造成强问法的手法
- **第零节「根因与必要性」闸门 + 11 类约 120 个问题**（`0.1`–`11.6`）—— 修 bug 前的根因与必要性 / 时序与并发 / 数据源与状态 / 约束的执行力 / 凭据密钥与生命周期 / 判据与证据 / 默认值兜底与未知态 / 复用漂移与文案 / 信任边界 / 资金 / 外部集成 / 用户体验
