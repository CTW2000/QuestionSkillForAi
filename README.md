...
- **13 条通用变换**（`T1`–`T13`）—— 把弱问法改造成强问法的手法
- **第零节「根因与必要性」闸门 + 12 类约 130 个问题**（`0.1`–`12.10`）—— 修 bug 前的根因与必要性 / 时序与并发 / 数据源与状态 / 约束的执行力 / 凭据密钥与生命周期 / 判据与证据 / 默认值兜底与未知态 / 复用漂移与文案 / 信任边界 / 资金 / 外部集成 / 用户体验 / 手写与引用（谁决定了引库还是手写、抽不抽象、会不会随升级失效）
